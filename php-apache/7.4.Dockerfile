...
RUN case ${TARGETARCH} in \
    ...
    apt-get update -y && \
    apt-get upgrade -y && \
    apt-get install -y --no-install-recommends git wget zip unzip openssh-client sudo inkscape imagemagick jpegoptim optipng webp ghostscript && \
    ...
    install-php-extensions amqp gd zip soap sockets bcmath imap intl \
        ...
        redis memcache memcached xdebug imagick mcrypt maxminddb && \
...
COPY configs/php7.4.ini $PHP_INI_DIR/php.ini
