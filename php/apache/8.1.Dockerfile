...
RUN case ${TARGETARCH} in \
    ...
    wget -q https://github.com/mlocati/docker-php-extension-installer/releases/latest/download/install-php-extensions -O /usr/local/bin/install-php-extensions && \
        ...
        sync && \
    install-php-extensions amqp gd zip soap sockets bcmath imap intl mysqli pdo_mysql xsl opcache pcntl shmop gettext calendar exif redis memcached xdebug imagick mcrypt maxminddb && \
...
COPY configs/php8.ini $PHP_INI_DIR/php.ini
COPY configs/apache.conf $APACHE_CONFDIR/sites-available/000-default.conf
...
RUN mkdir $APACHE_CONFDIR/ssl && \
    ...
    openssl req -new -x509 -days 1461 -nodes -out cert.pem -keyout cert.key -extensions req_ext -config /etc/ssl/developer.cnf && \
    chmod +x /usr/local/bin/entrypoint /usr/local/bin/copy-ssh-keys /usr/local/bin/add-host-alias /usr/local/bin/set-puid && \
