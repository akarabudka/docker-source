...
RUN apt-get update \
    ...
    && apt-get install -y --no-install-recommends \
        ...
        zip unzip openssh-client rsync sudo git wget curl apt-transport-https \
    && echo "deb https://packages.sury.org/php/ bullseye main" | tee /etc/apt/sources.list.d/sury-php.list \
    && wget -qO - https://packages.sury.org/php/apt.gpg | sudo apt-key add - \
#    && echo "deb https://deb.kelnik.ru/ bullseye main" | tee /etc/apt/sources.list.d/kelnik.list \
#    && wget -qO - https://deb.kelnik.ru/kelnik.gpg | sudo apt-key add - \
    && apt-get update -y \
    && apt-get install -y --no-install-recommends \
       ghostscript \
...
COPY configs/php/8.ini /etc/php/8.1/fpm/php.ini
COPY configs/php/8.ini /etc/php/8.1/cli/php.ini
COPY configs/php/www.conf /etc/php/8.1/fpm/pool.d/www.conf
COPY configs/php/docker.conf /etc/php/8.1/fpm/pool.d/docker.conf
COPY configs/php/zz-docker.conf /etc/php/8.1/fpm/pool.d/zz-docker.conf
